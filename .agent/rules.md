...
このファイルは、このプロジェクトにおけるルールと設定を定義します。

Antigravityはこれらの指示に従います。

## 全体的な方針
- 特段の指定がない限り、思考は英語で行って出力はわかりやすくて自然な日本語で出力してください
- 作成した要件定義書、設計書、タスクリストのマークダウンファイルは`spec`フォルダ配下に格納してください。フォルダが存在しない場合は作成してください。
- Spec駆動開発及び t-wadaさんのテスト駆動開発の原則に従って設計・開発を進めてください。
- 基本的に実装を始める前に必ず以下の内容を私に提示してください。
  - 選んだ実装方針の概要
  - 具体的な実装内容
  - その実装方針を選んだ理由
  - (もしあれば)懸念点やリスクなど
...
- **言語**: TypeScript
- **フォーマット**: biome
- **コメント**: 複雑なロジックには明確で簡潔なコメントを記述してください。

## アーキテクチャ
- 既存のディレクトリ構造に従ってください。
- コンポーネントは小さく、焦点を絞ったものにしてください。

## ワークフロー
- 変更を送信する前に必ずビルドコマンドとテストを実行してください。
- 機能変更時はドキュメントを更新してください。
